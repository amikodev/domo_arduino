...
void Domo_Pin_RF24::init(Domo_Pin *dp, RF24 *radio, CryptoDH *dh){
...
bool Domo_Pin_RF24::send(byte* buf, uint8_t len){
    bool ret = true;
    if(len > 0){
        RadioData rd;
...
bool Domo_Pin_RF24::sendCrypt(byte* buf, uint8_t len){
    bool ret = true;
    if(len > 0){
            ...
            */
            ...
            if(rd.page == 1 && rd.pagesCount == 1){
                if(dataType == RF24_CHK_P){
                    ...
                    buf[0] = CMD_RADIO;
                    buf[1] = CMD_TYPE_RADIO_K128;
                    buf[2] = 0;
                    memcpy(buf+3, (byte *)&K128, 8);
                    Serial.write(buf, sizeof(buf));
                    
                    buf[2] = 1;
                    memcpy(buf+3, ((byte *)&K128)+8, 8);
                    Serial.write(buf, sizeof(buf));
            ...
            if(rd.page == rd.pagesCount && dataType == RF24_CRYPTED_TEXT){
                ...
                _dp->radio_recieve(pipe_num, fullMessage, fullMessageLength);
...
void Domo_Pin_RF24::dump(byte* buf, uint8_t len, const char* title){
...
void Domo_Pin_RF24::print_value(char * str, byte * a, int bits){
